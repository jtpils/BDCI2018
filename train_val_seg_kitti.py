...
def main():
    ...
    label_ry_train, label_ry_reg_train = data_utils.load_seg_kitti(args.filelist)
    data_val, img_val, label_ry_val, data_num_val, label_val, label_hwl_val, label_xyz_val, \
    label_ry_reg_val = data_utils.load_seg_kitti(args.filelist_val)

    # shuffle

    data_train, img_train, data_num_train, label_train, label_hwl_train, label_xyz_train, \
    label_ry_train, label_ry_reg_train = data_utils.grouped_shuffle([data_train, img_train,
                                                                     data_num_train, label_train, label_hwl_train,
                                                                     label_xyz_train, label_ry_train,
                                                                     label_ry_reg_train])
    ...
    if setting.data_dim > 3:
        ...
        points, _, features = tf.split(pts_fts, [setting.data_format["pts_xyz"], setting.data_format["img_xy"],
                                                 setting.data_format["extra_features"]], axis=-1,
    ...
    logits, probs, logits_hwl, logits_xyz, logits_ry, probs_ry = \
        net.logits, net.probs, net.logits_hwl, net.logits_xyz, net.logits_ry, net.probs_ry
    ...
    #                                                     pf.ry_getangle(tf.nn.top_k(probs_ry, 1)[1],
    #                                                                    tf.zeros_like(labels_ry_reg)))))
    ...
    with tf.control_dependencies(update_ops):
        train_op = optimizer.minimize(loss_op + reg_loss + loss_hwl_op + loss_xyz_op + loss_ry_op
        ...
        for batch_idx in range(batch_num):
            if (batch_idx != 0 and batch_idx % step_val == 0) or batch_idx == batch_num - 1:
                ...
                for batch_val_idx in range(math.ceil(num_val / batch_size)):
                    ...
                    # 将loss 与 rmse/acc op 加入sess_op_list
                    sess_op_list = sess_op_list + [loss_hwl_op, loss_xyz_op, loss_ry_op, rmse_hwl_op, rmse_xyz_op,
                                                   t_1_acc_ry_op]
                    ...
                    sess_feed_dict = {pts_fts: points_batch,
                                      ...
                                      xforms: xforms_np, rotations: rotations_np, jitter_range: np.array([jitter_val]),
                                      labels_seg: labels_batch, labels_weights: weights_batch, is_training: False,
                                      labels_hwl: labels_hwl_batch, labels_xyz: labels_xyz_batch,
                                      labels_ry: labels_ry_batch, labels_ry_reg: labels_ry_reg_batch}

                    loss_val, t_1_acc_val, t_1_acc_val_instance, t_1_acc_val_others, loss_val_hwl, loss_val_xyz, \
                    loss_val_ry, rmse_hwl_val, rmse_xyz_val, t_1_acc_ry_val, mdis_ry_angle_val = \
                        sess.run(sess_op_list, feed_dict=sess_feed_dict)
                    # 输出iter结果
                    print('{}-[Val  ]-Iter: {:06d}  Loss: {:.4f} Loss_hwl: {:.4f} Loss_xyz: {:.4f} Loss_ry: {:.4f} '
                          'Rmse_hwl: {:.4f} Rmse_xyz: {:.4f} T-1 Ry_Acc: {:.4f} Mdis_ry_angle: {:.4f} T-1 Acc: {:.4f} '
                          .format(datetime.now(), batch_val_idx, loss_val, loss_val_hwl, loss_val_xyz, loss_val_ry,
                                  rmse_hwl_val, rmse_xyz_val, t_1_acc_ry_val, mdis_ry_angle_val, t_1_acc_val))
                ...
                # Tensorboard 可视化
                summaries_feed_dict = {loss_val_avg: loss_avg, t_1_acc_val_avg: t_1_acc_avg,
                                       t_1_acc_val_instance_avg: t_1_acc_instance_avg,
                                       t_1_acc_val_others_avg: t_1_acc_others_avg, loss_val_avg_hwl: loss_avg_hwl,
                                       loss_val_avg_xyz: loss_avg_xyz, loss_val_avg_ry: loss_avg_ry,
                                       rmse_val_hwl_avg: rmse_hwl_avg, rmse_val_xyz_avg: rmse_xyz_avg,
                                       t_1_acc_val_ry_avg: t_1_acc_ry_avg, mdis_val_ry_angle_avg: mdis_ry_angle_avg}

                summaries_val = sess.run(summaries_val_op, feed_dict=summaries_feed_dict)
                summary_writer.add_summary(summaries_val, batch_idx)

                # 输出平均结果
                print('{}-[Val  ]-Average:      Loss: {:.4f} Loss_hwl: {:.4f} Loss_xyz: {:.4f} Loss_ry: {:.4f} '
                      'Rmse_hwl: {:.4f} Rmse_xyz: {:.4f} T-1 Ry_Acc: {:.4f} Mdis_ry_angle: {:.4f} T-1 Acc: {:.4f} '
                      .format(datetime.now(), loss_avg, loss_avg_hwl, loss_avg_xyz, loss_avg_ry, rmse_hwl_avg,
                              rmse_xyz_avg, t_1_acc_ry_avg, mdis_ry_angle_avg, t_1_acc_avg))
            ...
            sess_feed_dict = {pts_fts: points_batch,
                              ...
                              xforms: xforms_np, rotations: rotations_np, jitter_range: np.array([jitter]),
                              labels_seg: labels_batch, labels_weights: weights_batch, is_training: True,
                              labels_hwl: labels_hwl_batch, labels_xyz: labels_xyz_batch, labels_ry: labels_ry_batch,
                              labels_ry_reg: labels_ry_reg_batch}

            _, loss, t_1_acc, t_1_acc_instance, t_1_acc_others, summaries, loss_hwl, loss_xyz, loss_ry, \
            rmse_hwl, rmse_xyz, t_1_acc_ry = sess.run(sess_op_list, feed_dict=sess_feed_dict)
            print('{}-[Train]-Iter: {:06d} Loss_seg: {:.4f} Loss_hwl: {:.4f} Loss_xyz: {:.4f} Loss_ry: {:.4f} '
                  'Rmse_hwl: {:.4f} Rmse_xyz: {:.4f} T - 1 Ry_Acc: {:.4f} T - 1 Acc:{:.4f}'.
                  format(datetime.now(), batch_idx, loss, loss_hwl, loss_xyz, loss_ry, rmse_hwl, rmse_xyz,
                         t_1_acc_ry, t_1_acc))
