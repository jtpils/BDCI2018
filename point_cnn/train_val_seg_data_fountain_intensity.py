...
def main():
    ...
    if args.filelist_val is None:
        ...
        data_train, intensity_train, data_num_train, label_train = data_utils.load_seg(args.filelist)
        data_val = data_train
        intensity_val = np.reshape(intensity_train, [data_val.shape[0], data_val.shape[1], 1])
        ...
        data_val, intensity_val, data_num_val, label_val = data_utils.load_seg(args.filelist_val)
        intensity_val = np.reshape(intensity_val, [data_val.shape[0], data_val.shape[1], 1])

    # shuffle
    data_train, intensity_train, data_num_train, label_train = \
        data_utils.grouped_shuffle([data_train, intensity_train, data_num_train, label_train])

    intensity_train = np.reshape(intensity_train, [data_train.shape[0], data_train.shape[1], 1])
    ...
    if setting.extra_dim > 0:
        ...
        points = pts
        features = fts
        #points, features = tf.split(pts_fts, [setting.data_format["pts_xyz"],  setting.data_format["extra_features"]], axis=-1, name='split_points_features')
        ...
        for batch_idx in range(batch_num):
            if (batch_idx != 0 and batch_idx % step_val == 0) or batch_idx == batch_num - 1:
                ...
                for batch_val_idx in range(math.ceil(num_val / batch_size)):
                    ...
                    intensity_batch = intensity_val[start_idx:end_idx, ...]
            ...
            intensity_batch = intensity_train[start_idx:end_idx, ...]
