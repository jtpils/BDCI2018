...
def load_seg_df(filelist):
    ...
    for line in open(filelist):
        ...
        intensities = np.reshape(intensities, (intensities.shape[0], intensities.shape[1], 1))
        points.append(np.concatenate((data['data'][...].astype(np.float32), intensities), axis=2))
        point_nums.append(data['data_num'][...].astype(np.int32))
